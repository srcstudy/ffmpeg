 ...
 *
gcc simplest_ffmpeg_muxer.cpp -g -o simplest_ffmpeg_muxer.out -I /usr/local/include -L /usr/local/lib -lavformat -lavcodec -lavutil -lm -lpthread -lz -lx264 -lx265 -lrt -ldl -lstdc++ -lvpx -lopenh264 -lswresample -lopus -lmp3lame -lfdk-aac -lbz2
...
int main(int argc, char* argv[]){
	...
	*/
	...
	if(argc < 4) {
		printf("usage: %s in_filename_v in_filename_a out_filename\n\n", argv[0]);
        return 1; 
	}
	in_filename_v 	  = argv[1];
	in_filename_a 	  = argv[2];
	out_filename 	  = argv[3];
...
#endif
	static int write_pkt_num = 0;
	static int print_v_pkt_times = 1;
	while (1) {
			...
			if(av_read_frame(ifmt_ctx, &pkt) >= 0){
				do{
					...
					if(pkt.stream_index == videoindex_v){
						...
						if(print_v_pkt_times <= 3){
							print_v_pkt_times++;
							printf("muxer:%d\n", pkt.size);
							showRawData( (char*)&pkt, pkt.size );
