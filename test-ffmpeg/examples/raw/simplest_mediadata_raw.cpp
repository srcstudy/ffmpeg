 ...
 * 
 * @param url        Location of PCM file.   drum.pcm
...
int simplest_pcm16le_cut_singlechannel(char *url,int start_num,int dur_num){
	...
	unsigned char *sample=(unsigned char *)malloc(2); //单声道

	int cnt=0;
	while(!feof(fp)){
		fread(sample,1,2,fp);
		if(cnt>start_num&&cnt<=(start_num+dur_num)){
			fwrite(sample,1,2,fp1); //2

			//取采样值
			short samplenum=sample[1];  //高位  LE
			samplenum=samplenum*256;
			samplenum=samplenum+sample[0];

			fprintf(fp_stat,"%6d,",samplenum);
			if(cnt%10==0)
				fprintf(fp_stat,"\n",samplenum);  //多写了一个samplenum
		}
		cnt++;
	}
...
int simplest_pcm16le_halfvolumeleft(char *url){
	...
	while(!feof(fp)){
		short *samplenum=NULL; // NOTE  
		fread(sample,1,4,fp);   //本来是2字节，但双声道*2

		samplenum=(short *)sample; //指向读出的数据,  
		*samplenum=*samplenum/2;   // short 代表2个字节
		//L
		fwrite(sample,1,2,fp1);
		//R
 ...
 * PCM16LE双声道音频采样数据转换为PCM8音频采样数据   NOTE
 * 
 * @param url  Location of PCM file.
 */
int simplest_pcm16le_to_pcm8(char *url){
	...
	while(!feof(fp)){
		...
		fread(sample,1,4,fp);
		//(-32768-32767)   to -128到127
		samplenum16=(short *)sample;
		samplenum8=(*samplenum16)>>8;  //除以256
		//(0-255)
		samplenum8_u=samplenum8+128;		
		//L
		fwrite(&samplenum8_u,1,1,fp1);//写1个字节




		samplenum16=(short *)(sample+2); //+2
